...
A simulated IoT smart-office system for a 3-room office (Drawing Room, Work Room 1, Work Room 2 — 2 fans + 3 lights per room, 18 devices total). One shared Node.js backend is the single source of truth; a real-time 3D web dashboard and a Discord bot both read and act on the same live (simulated) device state.
