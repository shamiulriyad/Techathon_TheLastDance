...
function Bookshelf() {
  return (
    <group position={[4.98, 0.72, -2.31]}>
      <mesh castShadow>
        <boxGeometry args={[0.82, 1.28, 0.2]} />
      ...
      {Array.from({ length: 9 }).map((_, index) => (
        <mesh key={index} position={[-0.3 + (index % 3) * 0.18, 0.38 - Math.floor(index / 3) * 0.34, 0.2]}>
          <boxGeometry args={[0.045, 0.2, 0.075]} />
...
function Furniture() {
  return (
    <>
      ...
      <WallDisplay position={[3.05, 1.55, -2.42]} rotation={[-Math.PI / 2, 0, 0]} />
      <Bookshelf />
